...
from utils_video2text import generate_dsg
import tqdm
from pprint import pprint
import time
from parse_utils import parse_tuple_output, parse_dependency_output, parse_question_output
# from video_qa_internvl import video_question_answering_internvl
# from video_qa_qwen import videoqa_qwen

from llm_api import openai_completion


def eval_video2text(data, save=False, n_parallel_workers=1, cache_dir="./results", llm=None, vqamodel=None):
    id2prompts = {}
    for i, item in enumerate(data):
        id2prompts[i] = {}
        id2prompts[i]['input'] = item['prompt']
        id2prompts[i]['video'] = item['video_path']
        id2prompts[i]['query'] = item['question']
    ...
    for key in tqdm.tqdm(id2prompts.keys()):
        try:
            ...
            # video_path = os.path.join(video_folder, id2prompts[key]['video'])
            ...
            print(e)
            results_vqa[key] = {"qid2answer": {}, "qid2scores": {}, "qid2validity": {},
                                "average_score_without_dep": 1,
                                "average_score": 1}
